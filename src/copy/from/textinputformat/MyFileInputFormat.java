 ...
 */
public abstract class MyFileInputFormat<K, V> implements InputFormat<K, V> {
  ...
  public InputSplit[] getSplits(JobConf job, int numSplits)
    throws IOException {
	  System.out.println("in getSplits of MyFileInputFormat");
    ...
    // generate splits
    ArrayList<MyFileSplit> splits = new ArrayList<MyFileSplit>(numSplits);
    ...
    for (FileStatus file: files) {
      ...
      if ((length != 0) && isSplitable(fs, path)) { 
        ...
        while (((double) bytesRemaining)/splitSize > SPLIT_SLOP) {
          ...
          int lengthSize = splitHosts.length;
          for (int i = 0; i < lengthSize; i++)
          {
        	  LOG.info("1 split hosts is " + splitHosts[i]);
          }
        ...
        if (bytesRemaining != 0) {
        	String[] splitHosts=blkLocations[blkLocations.length-1].getHosts();
        	int lengthSize = splitHosts.length;
            for (int i = 0; i < lengthSize; i++)
            {
          	  LOG.info("2 split hosts is " + splitHosts[i]);
            }
        ...
        for (int i = 0; i < lengthSize; i++)
        {
      	  LOG.info("3 split hosts is " + splitHosts[i]);
        }
        splits.add(new MyFileSplit(path, 0, length, splitHosts));
      } else { 
    ...
    return splits.toArray(new MyFileSplit[splits.size()]);
  }
